...
def getResult(game: HGroup, hypo: HGroup, action: ClueAction, log: Boolean = true): Double =
	...
	def distance5(g: HGroup, playerIndex: Int) =
		g.chop(playerIndex).flatMap: chop =>
			val oldest5 = state.hands(playerIndex).findLast: o =>
				val card = g.state.deck(o)

				o > chop &&
				card.rank == 5 &&
				!g.state.variant.suits(card.suitIndex).suitType.brownish &&
				!card.clued
	...
	val (badTouch, _, avoidableDupe) = badTouchResult(game, hypo, action)
...
private def forceSdcm(orig: HGroup, game: HGroup, playerIndex: Int, order: Int, offset: Int, knownTrash: Boolean): Double =
	...
	Log.info(s"${indent(offset)}${state.names(playerIndex)} ${Action.gerund(action)} ${state.logId(id)} (as sdcm):")
	...
	if stopLookahead then
		...
		playables.filter(o => meta(o).status == CardStatus.Finessed || game.isBlindPlaying(o)).minByOption(game.xmeta(_).turnFinessed.getOrElse(99)) match
			case Some(order) =>
				...
				Log.info(s"${indent(offset)}${state.names(playerIndex)} ${Action.gerund(action)} ${state.logId(id)} (f)")
		...
		if !state.canClue then
			val (order, isAnxiety) = player.anxietyPlay(state, playerIndex) match
				case Some(anxiety) if game.level >= Level.Stalling =>
					(anxiety, true)
				case _ =>
			...
			Log.info(s"${indent(offset)}${state.names(playerIndex)} ${Action.gerund(action)} ${state.logId(id)} ($order) from ${if isAnxiety then "anxiety" else "a locked hand"}!")
		...
		if state.canClue && (state.numPlayers > 2 || playerIndex == state.ourPlayerIndex) then
			...
			if clueVal < dcVal then
				...
				if sdcmValue > value then
					Log.highlight(Console.CYAN, f"${indent(offset)}prefer sdcm: $sdcmValue%.2f")
					sdcmValue
				else
					Log.highlight(Console.CYAN, f"${indent(offset)}overall: $value%.2f (clue%%: $clueProb)")
					value
		else
			Log.info(s"${indent(offset)}${state.names(playerIndex)} ${Action.gerund(action)} ${state.logId(id)}")
			advance(orig, dcGame, offset + 1)
...
def _evalAction(game: HGroup, action: Action): Double =
	...
	val value = action match
			...
			if hypoGame.lastMove == Some(ClueInterp.Mistake) || clueValue == -100 then
				...
				val value =
					if game.me.thinksPlayables(game, state.ourPlayerIndex).nonEmpty && game.inEndgame then
						-1 + 0.5 * clueValue
					else if game.me.thinksPlayables(game, state.ourPlayerIndex).exists(p => game.me.thoughts(p).id(infer = true).exists(id => game.me.hypoStacks(id.suitIndex) > id.rank)) then
						Log.highlight(Console.CYAN, "holding up a play sequence to clue!")
						-1 + 0.5 * clueValue
					else
						0.5 * clueValue
		...
		case PlayAction(playerIndex, order, _, _) =>
			...
			.when(_ != -100): total =>
				val average = total / uniqueInfs.length
...
def evalGame(orig: HGroup, game: HGroup, offset: Int): Double =
	...
	val log = List(
		Option.when(badCM != 0)(s"badCM: $badCM"),
		Option.when(playablesDiscarded != 0)(s"plays dc'd: $playablesDiscarded"),
		Option.when(lockedPenalty != 0)(s"locked: $lockedPenalty"),
		Option.when(endgamePenalty != 0)(s"endgame penalty: $endgamePenalty")
	)

	Log.info(f"${indent(offset)}state: $stateVal, future: $futureVal%.2f, bdr: $bdrVal touch: $touchVal%.2f ${log.flatten.mkString(" ")}")
